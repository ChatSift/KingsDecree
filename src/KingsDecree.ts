  ...
  public static async runDecreer(newKing: GuildMember | null = null) {
    if (newKing === null) {
      ...
      while (newKing === null || newKing?.id === KingsDecree.king?.id) {
        ...
        newKing = await KingsDecree.getGuild().members.fetch(randomElement(users)).catch(() => null);
        if (newKing) {
      ...
      components: [
        new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(
          new StringSelectMenuBuilder()
            ...
            .addOptions(
              KingsDecree.decrees.map((decree) => ({ label: decree.description, value: `decree-select:${decree.name}` })),
            ),
        ),
      ],
      embeds: [
